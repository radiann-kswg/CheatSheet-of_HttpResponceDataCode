...
- 機能:
  - ステータスコード一覧の検索と閲覧
  - Markdown本文の表示
  - 各ページの `http` コードブロック抽出（レスポンスサンプル表示）
  - サンプルのワンクリックコピー
...
3. `main` ブランチへ push すると、`.github/workflows/deploy-pages.yml` が実行されて公開される
4. Actions 完了後、表示された Pages URL へアクセスする

### ローカル確認

`index.html` をブラウザで開くか、簡易サーバーで確認できます（推奨）。
